"""E2E integration tests for tools.browser_supervisor.
...
These are real browser E2E tests — they launch Chrome, connect via
CDP WebSocket, and exercise the full supervisor lifecycle.  They are
NOT executed in normal test runs and require ``HERMES_E2E_BROWSER=1``.

Corresponding unit tests live in ``test_browser_supervisor.py`` and
run by default (no browser required).

Run manually:
    HERMES_E2E_BROWSER=1 scripts/run_tests.sh tests/tools/test_browser_supervisor_e2e.py
"""

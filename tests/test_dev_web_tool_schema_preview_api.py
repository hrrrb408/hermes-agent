...
class TestSchemaPreviewOpenAPI:
    ...
    def test_openapi_paths_31(self, client):
    ...
    def test_static_yaml_has_31_paths(self):
        """Verify the static YAML file has exactly 31 paths."""
        import pathlib
        import yaml
...
class TestSchemaPreviewRuntimeRoutes:
    ...
    def test_runtime_routes_31(self, client):
        resp = client.get("/openapi.json")
        spec = resp.json()
        paths = [p for p in spec["paths"] if p.startswith("/api/dev/v1/")]
        assert len(paths) == 31

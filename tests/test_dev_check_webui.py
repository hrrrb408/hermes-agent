...
def _minimal_valid_spec() -> dict[str, Any]:
    """Return a valid 34-path OpenAPI spec matching the Dev WebUI contract."""
    paths: dict[str, Any] = {
        ...
        "/memory/write/dry-run": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/memory/items/{memoryId}/update/dry-run": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/memory/items/{memoryId}/archive/dry-run": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/context/preview": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/agent/status": {"get": {"responses": {"200": {"description": "ok"}}}},
        "/agent/prompt/preview": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/agent/run/dry-run": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/agent/runs": {"post": {"responses": {"202": {"description": "accepted"}}}},
        "/agent/runs/{runId}": {"get": {"responses": {"200": {"description": "ok"}}}},
        "/agent/runs/{runId}/events": {"get": {"responses": {"200": {"description": "SSE stream"}}}},
        "/agent/runs/{runId}/cancel": {"post": {"responses": {"200": {"description": "ok"}}}},
        ...
        "/reviews/{reviewId}/approve/dry-run": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/reviews/{reviewId}/reject/dry-run": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/reviews/{reviewId}/approve/execute": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/reviews/{reviewId}/reject/execute": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/tools/policy": {"get": {"responses": {"200": {"description": "ok"}}}},
        "/tools/catalog": {"get": {"responses": {"200": {"description": "ok"}}}},
        "/tools/schemas": {"get": {"responses": {"200": {"description": "ok"}}}},
        "/tools/schemas/{canonicalName}": {"get": {"responses": {"200": {"description": "ok"}}}},
        "/tools/dry-run": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/tools/execute": {"post": {"responses": {"200": {"description": "ok"}}}},
        "/tools/audit-events": {"get": {"responses": {"200": {"description": "ok"}}}},
    }
    return {"openapi": "3.1.0", "info": {"title": "test", "version": "1.0"}, "paths": paths}


class TestWebuiCheckOpenapi:
    def test_valid_34_paths(self, tmp_path: Path) -> None:
        spec = _minimal_valid_spec()
        p = _write_openapi(tmp_path, spec)
        col = CheckCollector()
        _webui_check_openapi(col, p)
        assert col.by_label("OpenAPI paths") == ("PASS", "OpenAPI paths", "34")
        ...
        assert col.by_label("Tool policy routes") == ("PASS", "Tool policy routes", "5")

    def test_wrong_path_count(self, tmp_path: Path) -> None:
        spec = _minimal_valid_spec()
        # Remove one path to get 33
        del spec["paths"]["/agent/status"]
        p = _write_openapi(tmp_path, spec)
        col = CheckCollector()
        _webui_check_openapi(col, p)
        assert col.by_label("OpenAPI paths") == ("FAIL", "OpenAPI paths", "33")
    ...
    def test_forbidden_route_tools_dispatch(self, tmp_path: Path) -> None:
        spec = _minimal_valid_spec()
        spec["paths"]["/tools/dispatch"] = {"post": {"responses": {"200": {"description": "ok"}}}}
        p = _write_openapi(tmp_path, spec)
        col = CheckCollector()
        _webui_check_openapi(col, p)
        forbidden_check = col.by_label("Forbidden routes")
        assert forbidden_check is not None
        assert forbidden_check[0] == "FAIL"
        assert "/tools/dispatch" in forbidden_check[2]
    ...
    def test_35_paths(self, tmp_path: Path) -> None:
        spec = _minimal_valid_spec()
        spec["paths"]["/extra"] = {"get": {"responses": {"200": {"description": "ok"}}}}
        p = _write_openapi(tmp_path, spec)
        col = CheckCollector()
        _webui_check_openapi(col, p)
        assert col.by_label("OpenAPI paths") == ("FAIL", "OpenAPI paths", "35")

    def test_34_paths_all_present_forbidden_absent(self, tmp_path: Path) -> None:

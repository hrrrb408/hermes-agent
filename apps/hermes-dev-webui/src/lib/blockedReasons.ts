...
const CATALOGUE: Readonly<Record<string, BlockedReasonInfo>> = {
  ...
  // ── Workflow surface (Phase 3A) ──
  blocked_workflow_real_provider_not_allowed: {
    code: 'blocked_workflow_real_provider_not_allowed',
    title: 'Real provider not allowed',
    explanation: 'A workflow step attempted to use the real provider. Phase 3A permits only the offline fake provider.',
    safeNextAction: 'Use the fake provider round-trip step; real provider stays blocked by design.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_autonomous_write_not_allowed: {
    code: 'blocked_workflow_autonomous_write_not_allowed',
    title: 'Autonomous write not allowed',
    explanation: 'A workflow step attempted an autonomous write or write execution. The workflow only carries write previews.',
    safeNextAction: 'Use the sandbox write preview step; execute the write separately through the existing write confirmation flow.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_provider_write_not_allowed: {
    code: 'blocked_workflow_provider_write_not_allowed',
    title: 'Provider write not allowed',
    explanation: 'A provider-suggested write tool appeared in a workflow step, which is a provider-write bid and is blocked.',
    safeNextAction: 'Restrict provider steps to read-only tools; provider writes are preview-only by design.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_rollback_execute_not_allowed: {
    code: 'blocked_workflow_rollback_execute_not_allowed',
    title: 'Rollback execute not allowed',
    explanation: 'A workflow step attempted a rollback execution. The workflow only references/ previews rollbacks.',
    safeNextAction: 'Use the rollback reference step; execute the rollback separately through the existing rollback confirmation flow.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_shell_not_allowed: {
    code: 'blocked_workflow_shell_not_allowed',
    title: 'Shell command not allowed',
    explanation: 'A workflow step attempted to run a shell command, which is permanently blocked in Phase 3A.',
    safeNextAction: 'Remove the shell step; no shell execution is permitted from the workflow.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_database_not_allowed: {
    code: 'blocked_workflow_database_not_allowed',
    title: 'Database access not allowed',
    explanation: 'A workflow step attempted a database query or mutation, which is permanently blocked in Phase 3A.',
    safeNextAction: 'Remove the database step; no database access is permitted from the workflow.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_external_service_not_allowed: {
    code: 'blocked_workflow_external_service_not_allowed',
    title: 'External service not allowed',
    explanation: 'A workflow step attempted an external HTTP / service write, which is permanently blocked in Phase 3A.',
    safeNextAction: 'Remove the external service step; no external service write is permitted from the workflow.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_production_not_allowed: {
    code: 'blocked_workflow_production_not_allowed',
    title: 'Production operation not allowed',
    explanation: 'A workflow step targeted a production operation, ~/.hermes, or production state.db — permanently blocked.',
    safeNextAction: 'Remove the production step; the workflow is dev-only and may never touch production.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_plugin_dynamic_load_not_allowed: {
    code: 'blocked_workflow_plugin_dynamic_load_not_allowed',
    title: 'Dynamic plugin load not allowed',
    explanation: 'A workflow step attempted a dynamic plugin / code load, which is permanently blocked in Phase 3A.',
    safeNextAction: 'Remove the dynamic-load step; no dynamic plugin loading is permitted from the workflow.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_step_type_not_allowed: {
    code: 'blocked_workflow_step_type_not_allowed',
    title: 'Step type not allowed',
    explanation: 'The step type is not one of the six permitted Phase 3A workflow step types.',
    safeNextAction: 'Use an allowed step type (read-only tool, fake provider, sandbox write preview, rollback reference, manual note, or audit query).',
    severity: 'warn',
    surface: 'workflow',
  },
  blocked_workflow_approval_required: {
    code: 'blocked_workflow_approval_required',
    title: 'Approval required',
    explanation: 'The step requires an explicit, single-use approval token before it can execute.',
    safeNextAction: 'Preview the step to issue the approval token, then execute it.',
    severity: 'info',
    surface: 'workflow',
  },
  blocked_workflow_approval_expired: {
    code: 'blocked_workflow_approval_expired',
    title: 'Approval expired',
    explanation: 'The step approval token TTL has elapsed.',
    safeNextAction: 'Re-preview the step to issue a fresh approval token.',
    severity: 'warn',
    surface: 'workflow',
  },
  blocked_workflow_approval_scope_mismatch: {
    code: 'blocked_workflow_approval_scope_mismatch',
    title: 'Approval scope mismatch',
    explanation: 'The approval token scope does not match the workflow step approval scope.',
    safeNextAction: 'Re-preview the step to issue a correctly scoped approval token.',
    severity: 'warn',
    surface: 'workflow',
  },
  blocked_workflow_approval_step_mismatch: {
    code: 'blocked_workflow_approval_step_mismatch',
    title: 'Approval step mismatch',
    explanation: 'The approval token was issued for a different step or execution.',
    safeNextAction: 'Re-preview the intended step to issue a matching approval token.',
    severity: 'warn',
    surface: 'workflow',
  },
  blocked_workflow_approval_digest_mismatch: {
    code: 'blocked_workflow_approval_digest_mismatch',
    title: 'Approval digest mismatch',
    explanation: 'The step input changed after approval, invalidating the approval token.',
    safeNextAction: 'Re-preview the step with the final input to issue a matching approval token.',
    severity: 'warn',
    surface: 'workflow',
  },
  blocked_workflow_approval_already_used: {
    code: 'blocked_workflow_approval_already_used',
    title: 'Approval already used',
    explanation: 'The single-use approval token has already been consumed.',
    safeNextAction: 'Re-preview the step to issue a new single-use approval token.',
    severity: 'warn',
    surface: 'workflow',
  },
  blocked_workflow_unsafe_path_not_allowed: {
    code: 'blocked_workflow_unsafe_path_not_allowed',
    title: 'Unsafe path not allowed',
    explanation: 'A step input contained an absolute path, traversal sequence, or production path.',
    safeNextAction: 'Use a sandbox-relative path with no traversal or production references.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_secret_input_not_allowed: {
    code: 'blocked_workflow_secret_input_not_allowed',
    title: 'Secret input not allowed',
    explanation: 'A step input resembled a secret (API key / bearer token / private key).',
    safeNextAction: 'Remove the secret material from the step input; the workflow never accepts secrets.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_raw_token_input_not_allowed: {
    code: 'blocked_workflow_raw_token_input_not_allowed',
    title: 'Raw token input not allowed',
    explanation: 'A step input carried a raw token / hash field, which the workflow never accepts.',
    safeNextAction: 'Remove the raw token / hash field from the step input.',
    severity: 'danger',
    surface: 'workflow',
  },
  blocked_workflow_invalid_input: {
    code: 'blocked_workflow_invalid_input',
    title: 'Invalid workflow input',
    explanation: 'A step input failed validation (wrong shape, too long, or missing a required field).',
    safeNextAction: 'Correct the step input and re-build the plan.',
    severity: 'warn',
    surface: 'workflow',
  },
  blocked_workflow_store_unavailable: {
    code: 'blocked_workflow_store_unavailable',
    title: 'Workflow store unavailable',
    explanation: 'The dev-only workflow store is unavailable (no dev HERMES_HOME or a confinement failure).',
    safeNextAction: 'Run the dev console against the dev HERMES_HOME so the workflow store is available.',
    severity: 'warn',
    surface: 'workflow',
  },
}

/**
 * Phase 4B — Target B Implementation route-governance tests (frontend).
 *
 * Asserts the Target B Implementation region is a read-only client-side region
 ...
 * redirect. No /target-b-implementation, /target-b-runtime, or /execute route
 * was added. The section registry is unchanged (Target B Implementation lives
 * inside Governance Hub). Cross-link navigation from the region stays a
 * client-side section switch (no SPA / backend route change).
 */
...
describe('Target B Implementation route governance (Phase 4B)', () => {
  beforeEach(() => {
  ...
  it('no SPA route named or shaped for target b implementation / runtime / execute was added', () => {
    for (const r of routes) {
      const path = String(r.name ?? '') + ' ' + String(r.path ?? '')
      expect(path).not.toMatch(/target[-_]?b[-_]?implementation/i)
      expect(path).not.toMatch(/target[-_]?b[-_]?runtime/i)
      expect(path).not.toMatch(/\/execute/i)
      expect(path).not.toMatch(/plugin[-_]?execute/i)
  ...
  it('Governance Hub remains a first-class DevConsole section; no new section was added', () => {
    // The section registry is unchanged — Target B Implementation lives INSIDE
    // the Governance Hub.
  ...
  it('the Target B Implementation region renders inside the Governance Hub section graph', () => {
    ...
    const text = wrapper.find('[data-testid="governance-hub-target-b-impl-region"]').text()
    expect(text).toContain('Target B')
    expect(text).toContain('IMPLEMENTATION')
  })

  it('the Target B Readiness region (Phase 4A) still renders intact alongside the implementation region', () => {
    const wrapper = mount(GovernanceHubSection)
  ...
  it('cross-link navigation from the Target B Implementation region stays a client-side section switch', async () => {
    const nav = useDevConsoleNavStore()
    const before = router.currentRoute.value.name
    nav.setSection('governanceHub')
    const wrapper = mount(GovernanceHubSection)
    await wrapper.find('[data-testid="governance-hub-target-b-impl-view-runtime-governance"]').trigger('click')
    expect(nav.activeSection).toBe('runtimeGovernance')
    await wrapper.find('[data-testid="governance-hub-target-b-impl-view-human-review"]').trigger('click')

...
- Phase 1F Release Fix 4: Completed
- Phase 1F-Release: Pending final re-verification
- Phase 1G: Not Started

---

## 22. Release Fix 4 — Cancel Transition Race, SSE Lifecycle, Browser Smoke (2026-06-09)

### Cancel Transition Race Root Cause

The cancel handler in `AgentRunService.cancel_run()` performed state reads
and writes across **multiple separate lock acquisitions**:

1. `get_run()` → lock #1 (read)
2. `append_event(RUN_CANCELLING)` → lock #2 (write)
3. `mark_cancelling()` → lock #3 (write)

Between acquisitions, the Worker could complete/fail the run, causing
`mark_cancelling()` or `cancel_run_completed()` to encounter an unexpected
state and raise `TransitionError`. This propagated as HTTP 500.

### Atomic request_cancel Fix

Added `AgentRunRegistry.request_cancel()` — a single method that performs
ALL state reads, decisions, and writes within **one lock acquisition**.
Returns a `CancelDecision` dataclass containing everything the Service
needs to act on, without touching Registry state again.

The Service cancel handler now:
1. Calls `request_cancel()` (atomic)
2. Emits `run.cancelling` event (if decision says to)
3. Interrupts agent (best-effort)
4. Waits for worker future with timeout
5. Emits `run.cancelled` only if worker didn't already emit terminal

### Worker Terminal Event Fix

Worker `_worker()` now checks the actual status after `complete_run()`.
If `complete_run()` transitioned to CANCELLED (cancel takes priority),
the worker emits `RUN_CANCELLED` instead of `RUN_COMPLETED`.

### Cancel Idempotency

| Current State | Cancel Returns |
|---|---|
| CREATED | 200, status → CANCELLED |
| STARTING | 200, status → CANCELLING |
| RUNNING | 200, status → CANCELLING |
| CANCELLING | 200, alreadyRequested=true |
| COMPLETED | 200, alreadyTerminal=true |
| CANCELLED | 200, alreadyTerminal=true |
| FAILED | 200, alreadyTerminal=true |
| Nonexistent run | 404 |

No state ever returns 500 from cancel.

### SSE First Connection Replay

Fixed `stream_run_events()` to replay ALL buffered events when no
`Last-Event-ID` is provided. Previously, first connections only received
events emitted *after* connection, missing `run.created` and `run.started`.

Now `after_sequence=0` is used for first connections, ensuring:
- `run.created` (sequence 1) is always delivered
- `run.started` (sequence 2) is always delivered

### Browser Smoke Strengthening

- `run.created` is now a mandatory assertion (first event)
- `run.started` is now a mandatory assertion (before deltas)
- `message.completed` must come after last `message.delta`
- Cancel HTTP MUST be 200 (no more accepting 500)
- Full delta text must match `EXPECTED_FINAL_TEXT`
- Fake Provider adds `initial_delay=0.15s` for SSE connection stability

### Session Precise Counting

Success smoke asserts `messageDelta === 2` (exactly 1 user + 1 assistant).
Cancel smoke asserts `messageDelta <= 2`.

### Terminal Event Uniqueness

- Worker emits exactly one terminal event (RUN_COMPLETED or RUN_CANCELLED)
- Cancel handler checks `terminal_event_emitted` before emitting
- `_ensure_terminal_event()` in worker `finally` block is safety net
- Total terminal events across all sources: exactly 1

### Cancel Race Tests (8 new)

| Test | Iterations | Assertion |
|---|---|---|
| Worker Complete vs Cancel | 50 | Cancel always 200, never 500 |
| Worker Fail vs Cancel | 50 | Cancel always 200, never 500 |
| Double Cancel | 30 | Both 200, one alreadyRequested |
| Cancel COMPLETED | 1 | 200 alreadyTerminal |
| Cancel CANCELLED | 1 | 200 alreadyTerminal |
| Cancel FAILED | 1 | 200 alreadyTerminal |
| Cancel Timeout Resources | 1 | Session lock retained |
| Worker Exit After Timeout | 1 | Resources released |

### SSE Lifecycle Tests (5 new)

| Test | Assertion |
|---|---|
| run.created sequence | sequence = 1 |
| run.started after created | sequence > created |
| First connection replay | ALL events from sequence 0 |
| Delta after started | First delta after run.started |
| Complete lifecycle | Ordered subsequence enforced |

---

## 23. Next Task

Phase 1F-Release: 最终完整封板核验与推送准备.
